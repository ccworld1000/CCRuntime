...
@interface CCObjCProtocol : CCProtocol
{
    Protocol *_protocol;
}
...
- (id)initWithObjCProtocol: (Protocol *)protocol
{
    return [[CCObjCProtocol alloc] initWithObjCProtocol: protocol];
}
...
{
    return [other isKindOfClass: [CCProtocol class]] &&
...
- (NSArray *)methodsRequired: (BOOL)isRequiredMethod instance: (BOOL)isInstanceMethod
{
    ...
    for(unsigned i = 0; i < count; i++)
    {
        ...
        [array addObject: [CCMethod methodWithSelector: methods[i].name implementation: NULL signature: signature]];
    }
